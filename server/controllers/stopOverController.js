...
exports.render = async (req, res) => {
    try {
        ...
        res.status(200).json({ response: { status: "success", data: { message: "", payload: stopOvers } } })
    } catch (error) {
        console.log(error)
        res.status(500)
    }
...
exports.create = async (req, res) => {
    try {
        ...
        res.status(201).json({ response: { status: 'success', data: { message: "escale Ajouter avec succès", payload: stopOver } } })
    } catch (error) {
        if (error.name === "SequelizeValidationError") {
            return res.status(400).json({ response: { status: "failed", data: error.errors } })
        }
        console.log(error)
        res.status(500).json({ response: { status: 'failed', data: "une erreur s'est produite veuillez contacter l'admin" } })
    }
}
...
exports.remove = async (req, res) => {
    try {
        ...
        res.status(200).json({ response: { status: 'success', data: { message: "Opération effectuer avec succès", payload: affectedRowsCount } } })
    } catch (error) {
        console.log(error)
...
exports.update = async (req, res) => {
    try {
        ...
        if (stopDate && stopOverToUpdate.stopDate.getTime() !== new Date(stopDate).getTime()) {
        ...
        res.status(200).json({ response: { status: 'success', data: { message: "", payload: updatedStopOver } } })
    } catch (error) {
        res.status(500).json({ response: { status: 'failed', data: "une erreur s'est produite veuillez contacter l'admin", payload: error } })
    }
}